...
namespace Chunk
{
	...
	public partial class Chunk : StaticBody3D
	{
		...
		{
			Logger<Chunk> logger = new(new FileService());
			for (int x = 0; x < dimensions.X; x++)
			{
				for (int y = 0; y < dimensions.Y; y++)
				{
					for (int z = 0; z < dimensions.Z; z++)
					{
						...
						{
							logger.Log(LogStatus.ERROR, "Error block instantiate: ", ex);
							block = new();
							logger.Log(LogStatus.OK, $"{BlockManager.Instance.Dirt.Texture.ResourceName}");
						}
						_blocks[x, y, z] = block;
					}
				}
